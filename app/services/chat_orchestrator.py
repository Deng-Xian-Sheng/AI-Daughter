...
async def handle_user_message(session_id: str, user_text: Optional[str]):
    ...
    if user_text:
        ...
        if need:
            from app.services.prompt_writer import build_text2img_prompt
            if mode == "img2img":
                pos2, neg2 = build_text2img_prompt({"who":"女孩","where":"卧室","action":"与描述相符的动作","props":"家居小物","lighting":"暖光","framing":"中景","style":"写实"})
                task_id = await run_image_task("text2img", pos2, neg2, "auto", None, None, None)
            else:
                pos, neg = build_text2img_prompt({"who":"场景元素","where":"与对话相关地点","action":"关键行为","props":"相关道具","lighting":"自然","framing":"中景","style":"写实"})
            ...
            # 任务完成后落库图片消息（见 _watch_and_publish_task 内）
